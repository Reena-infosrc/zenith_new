...
@router.get("/{admin_id}", response_model=Admin)
async def get_admin(admin_id: str, current_user: dict = Depends(get_current_active_user)):
    if not await is_user_admin(current_user["email"]):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only admins can access this endpoint"
...
@router.put("/{admin_id}", response_model=Admin)
async def update_admin(admin_id: str, admin_data: AdminUpdate, current_user: dict = Depends(get_current_active_user)):
    if not await is_user_admin(current_user["email"]):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only admins can update admins"
...
@router.delete("/{admin_id}")
async def delete_admin(admin_id: str, current_user: dict = Depends(get_current_active_user)):
    if not await is_user_admin(current_user["email"]):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only admins can delete admins"
...
@router.get("/auth-admins", include_in_schema=True)
@router.get("/auth-admins/", include_in_schema=True)
async def public_get_admins():
    try:
        table = await get_admins_table()
        ...
        logger.error(f"Public get_admins error: {str(e)}")
        return JSONResponse(
            content={"admins": [], "count": 0, "error": str(e)},
            status_code=500,
        )

@router.get("/auth-admins-check/{email}", include_in_schema=True)
@router.get("/auth-admins-check/{email}/", include_in_schema=True)
async def public_check_admin(email: str):
    try:
        ...
        is_admin = len(response.get("Items", [])) > 0
        return JSONResponse(
            content={
                "email": decoded_email,
                "is_admin": is_admin,
        ...
        logger.error(f"Public check_admin error for {email}: {str(e)}")
        return JSONResponse(
            content={
                "email": email,
                "is_admin": False,
